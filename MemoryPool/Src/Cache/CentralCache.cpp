...
namespace DM
{
	...
	{ 
        ...
        try 
        {
            ...
            if (!result)
            {
				...
				size_t allocBlocks = min(batchNum, totalBlocks);
                //构建链表将每个内存块的下一个内存块的地址写入到前8个字节
                if (allocBlocks > 1)
                {  // 确保至少有两个块才构建链表
                    ...
                    *reinterpret_cast<void**>(start + (allocBlocks - 1) * size) = nullptr;//置空最后一个内存块指向的下一个内存块指针
                }
				//将多余的内存块存在空闲列表中
            ...
            else 
            {
				...
				batchNum = count;
				if (prev) // 当前_CentralFreeList[index]链表上的内存块大于batchNum时需要用到 
				{
	...
	void CentralCache::ReturnRange(void* start, size_t size, size_t index)
	{
		...
		try
		{
			...
			*reinterpret_cast<void**>(end) = current;  // 将原链表头接到归还链表的尾部
			_CentralFreeList[index].store(start, std::memory_order_release);  // 将归还的链表头设为新的链表头
		}
		catch (...)
		{
			_Locks[index].clear(std::memory_order_release);
	...
	void* CentralCache::FetchFromPageCache(size_t size)
	{
		...
		{
			// 小于等于32KB的请求，使用固定8页
			return PageCache::Inst()->AllocateSpan(SPAN_PAGES);
		}
		else
		{
			// 大于32KB的请求，按实际需求分配
			return PageCache::Inst()->AllocateSpan(numPages);
		}
	}
